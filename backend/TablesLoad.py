...
def loadSocialNetwork(engine):
    ...
    df = pd.read_csv('./data/SocialNetwork.csv')
...
def loadPubs(engine):
    ...
    df = pd.read_csv('./data/Pubs.csv').drop(columns={'buildingId'})
...
def loadRestaurants(engine):
    ...
    df = pd.read_csv('./data/Restaurants.csv').drop(columns={'buildingId'})
    df['locationX'] = df['location'].apply(lambda x: x.replace(' ', '' , 1)).apply(lambda x: x[x.index('(')+1 : x.index(' ')])
    df['locationY'] = df['location'].apply(lambda x: x.replace(' ', '' , 1)).apply(lambda x: x[x.index(' ')+1 : x.index(')')])
    df.drop(columns={'location'}, inplace=True)
    print(df.head())
...
def loadLocationLookup(engine):
    ...
    aptDf = pd.read_csv('./data/Apartments.csv')[['apartmentId','location']].rename(columns={"apartmentId": "locationId"})
    empDf = pd.read_csv('./data/Employers.csv')[['employerId','location']].rename(columns={"employerId": "locationId"})
    pubsDf = pd.read_csv('./data/Pubs.csv')[['pubId','location']].rename(columns={"pubId": "locationId"})
    restaurantDf = pd.read_csv('./data/Restaurants.csv')[['restaurantId','location']].rename(columns={"restaurantId": "locationId"})
    schoolDf = pd.read_csv('./data/Schools.csv')[['schoolId','location']].rename(columns={"schoolId": "locationId"})
...
def loadTravelJournal(engine):
    ...
    df = pd.read_csv('./data/TravelJournal.csv')
...
def loadEmployers(engine):
    ...
    df = pd.read_csv('./data/Employers.csv').drop(columns={'buildingId'})
    df['locationX'] = df['location'].apply(lambda x: x.replace(' ', '' , 1)).apply(lambda x: x[x.index('(')+1 : x.index(' ')])
    df['locationY'] = df['location'].apply(lambda x: x.replace(' ', '' , 1)).apply(lambda x: x[x.index(' ')+1 : x.index(')')])
    df.drop(columns={'location'}, inplace=True)
    print(df.head())
...
def loadParticipantStatusLogs(engine):
    ...
    path = './data/ParticipantStatusLogs'
